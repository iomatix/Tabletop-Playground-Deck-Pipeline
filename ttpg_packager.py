...
hierarchical context-aware naming, and unified TTPG Card template generation.
"""
...
class ContextResolver:
    ...
    def resolve_deck_context(self, deck_dir: Path) -> tuple[DeckMetadata, dict[str, Any]]:
        ...
        # Read contract generated by deck_processor.py
        ...
        # Build informative, collision-free display name
...
class TemplateFactory:
    """Constructs TTPG Card templates with multi-sheet and contextual naming."""
    ...
    def create_card_template(
        ...
        tooltip = f"{meta.package_context or meta.universe} • {meta.deck_name}"

        if meta.is_document:
            description = (
                f"Official document/rulesheet: '{meta.deck_name}'.\n"
                f"Package: {meta.package_context or meta.universe}.\nPages: {total_items}."
            )
        else:
            description = (
                f"Card deck: '{meta.deck_name}'.\n"
                f"Package: {meta.package_context or meta.universe}.\nTotal cards: {total_items}."
            )
        ...
        for slot_idx, pair in enumerate(pairs):
            ...
            if meta.is_document:
                card_names[slot_key] = f"{meta.deck_name} - Page {slot_idx + 1}"
                card_metadata[slot_key] = json.dumps({"page": slot_idx + 1}, ensure_ascii=False)
            else:
...
class PackageOrchestrator:
    ...
    def process_deck(self, deck_dir: Path) -> None:
        ...
        print(
            ...
            f"Type: {'Document' if meta.is_document else 'Deck'} | Grid: {grid.cols}x{grid.rows} | Sheets: {total_sheets}"
        )

        # Prefix texture filename with package context to eliminate file collisions in Textures/
        ...
        print(f"  [+] Template generated: {template_file.name}")
